async function testPlymouth() {
  const h = { hotelName: 'The Plymouth Hotel', city: 'Miami Beach', neighborhood: 'South Beach' };
  console.log(`\n========================================`);
  ...
  (data.ota_conversion_audit?.optimal_5_photo_sequence || []).forEach(s => {
    ...
    console.log(`  Action Rationale: ${s.upgrade_rationale}`);
    console.log(`  URL: ${s.photo_url}`);
  });
}

testPlymouth().catch(console.error);
