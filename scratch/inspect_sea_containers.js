async function inspectSeaContainers() {
  const res = await fetch('http://localhost:3002/api/master-vibe-audit', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ hotelName: 'Sea Containers London', city: 'London', neighborhood: 'South Bank' })
  });
  const data = await res.json();
  console.log('=== SEA CONTAINERS LONDON: LIVE BOOKING.COM PHOTOS ===');
  (data.ota_conversion_audit?.live_photos || []).forEach((p, idx) => {
    console.log(`Live Photo #${p.slot || (idx + 1)}: Title: "${p.title}" | URL: ${p.imageUrl}`);
  });

  console.log('\n=== SEA CONTAINERS LONDON: OPTIMAL 5-SLOT SEQUENCE ===');
...
inspectSeaContainers().catch(console.error);
